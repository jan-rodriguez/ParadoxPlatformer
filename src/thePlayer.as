package 
{
	...
	public class thePlayer extends Entity
	{
		...
		private var animPosition:int = 2;
		private var framePosition:int = 3;
		private var flippedPosition:int = 4;
		...
		override public function update():void {
			...
			if (!Input.check(Key.SHIFT) || !rewindState) 
			{
				...
				{
				playerPosition.push([x, y, currentAnim, doxSprite.frame, flipped]);
				previousX = x; 
				previousY = y;
				}
			...
			if (!rewindState) {
				...
				doxSprite.flipped = flipped;
				// Resume animation from where it was without resetting the frame
				// (Over time, since doxSprite.frame is going down, this will result
				// in the animation rewinding)
				doxSprite.play(currentAnim, false, doxSprite.frame);
			}
		}
